...
TRANSFORMS = [
    ('scaled', '{}x.png', [1, 1.1, 1.25, 1.5, 1.75, 2], inverse_scale, 'Scale amount'),
...
for base_image in BASE_IMAGES:
    for transform_name, file_template, transform_vals, inverse_transform, transform_axis_label in TRANSFORMS:
        ...
        # locations and orientations the keypoints "should have" if SIFT was perfectly scale/rotation invariant.
        # Finally, we select keypoints that match in location and orientation across all versions of the image.
        ...
        for i in range(1, image_versions):
            for j, (orig_keypoint, _) in enumerate(keypoints[0]):
                matches = [kp for kp, adjusted_kp in keypoints[i]
                           if numpy.linalg.norm(orig_keypoint.pt - adjusted_kp[0]) <= KEYPOINT_MATCH_DIST_THRESHOLD
                           and abs(orig_keypoint.angle - adjusted_kp[1]) <= KEYPOINT_MATCH_ANGLE_THRESHOLD]
                if matches:
                    # Only need one match, so choose the strongest keypoint.
                    match = max(matches, key=lambda kp: kp.response)
